...
function rewriteInternalUrl($url, $pagesDir, $currentDestRelative)
{
    ...
    if ($path === '' || $path === '/') {
        $newPath = makeRelativePath($currentDestRelative, 'index.php');
        if ($query !== '') {
            $newPath .= '?' . $query;
        }
        if ($fragment !== '') {
    ...
    if (preg_match('~^(api|public)/~', $relativePath)) {
        $newPath = makeRelativePath($currentDestRelative, $relativePath);
        if ($query !== '') {
            $newPath .= '?' . $query;
        }
        if ($fragment !== '') {
 ...
 * Used during build time. Handles: href, action, src attributes.
 */
...
function generateEntrypoint($destPath, $pageRelativePath, array $paramNames)
{
    ...
    if (!empty($paramNames)) {
        ...
        $code .= "                \$params[\$key] = htmlspecialchars(\$_GET[\$key], ENT_QUOTES, 'UTF-8');\n";
        $code .= "            }\n";
        $code .= "        }\n";
        $code .= "        return \$params;\n";
        $code .= "    }\n";
